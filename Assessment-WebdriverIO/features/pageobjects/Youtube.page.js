...
class YoutubePage{
    ...
    async verifyHomePage(){
        helper.assertWithAllure("Verifying the page title and url",async()=>{
    ...
    async enterSearchQuery(){
        helper.assertWithAllure("Validating the visibility and enability of the search bar",async()=>{
            expect(await this.searchColumn.isDisplayed()).to.be.true;
            expect(await this.searchBar.isEnabled()).to.be.true;
        });
        helper.assertWithAllure("Entering the search query and clicking search button",async()=>{
    ...
    async validateSearchResults(){
        helper.assertWithAllure("Validating the search results to be greater than expected",async()=>{
    ...
    async clickOnVideo(){
        helper.assertWithAllure("Selecting any video from the search results",async()=>{
            await this.selectedVideo.click();
            await browser.pause(parseInt(process.env.smallTimeout));
        });
    }
    async handlePopUps(){
        helper.assertWithAllure("Validating the popups visibility and handling them",async()=>{
    ...
    async interactWithVideoControls(){
        helper.assertWithAllure("Validating the visibility and enability of the play/pause button",async()=>{
            expect(await this.playPauseButton.isDisplayed()).to.be.true;
            expect(await this.playPauseButton.isEnabled()).to.be.true;
        });
        helper.assertWithAllure("Clicking play/pause button multiple times",async()=>{
            let count = 1;
            while (count < 5) {
        ...
        helper.assertWithAllure("Clicking the video setting button",async()=>{
